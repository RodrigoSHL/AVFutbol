...
class TraerDatos{
	...
	protected function traer_nombres(){
		...
		$consulta = "select * from hoteles.prueba_app";
	...
	protected function actualizar(){
		...
		$act = "update hoteles.prueba_app set estado = 1";
	...
	protected function eliminar(){
		...
		$eliminar = "update hoteles.prueba_app set estado = 1 where id = ".$this->info["id"]." ";
	...
	protected function traer_info(){
		...
		if($this->info["tipo"] == 1)
			$consulta = "select * from hoteles.prueba_app where estado = 0 order by creacion desc";
		else{
			...
			if(!empty($this->info["desde"]) and !empty($this->info["hasta"])){
				$desde = date("Y-m-d",strtotime($this->info["desde"]));
				$hasta = date("Y-m-d",strtotime($this->info["hasta"]));
				$consulta = "select * from hoteles.prueba_app where DATE_FORMAT(creacion,'%Y-%m-%d') between '".$desde."' and '".$hasta."' order by creacion desc";
			}else
				$consulta = "select * from hoteles.prueba_app order by creacion desc";
		}
